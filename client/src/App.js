...
function App() {
  ...
  //bir web 3 provider, standart bir web 3 sağlayıcısını sarar; bu, metamask'ın her papera window.ethereum olarak enjekte ettiği şeydir
  const provider = new ethers.providers.Web3Provider(window.ethereum);

  //metamask eklentisi ayrıca imzalama işlemlerinin ether göndermesine ve blokcahin içindeki durumu değiştirmek için ödeme yapmasına izin verir. bunun için account signerına (imzalayıcısına) ihtiyacınız var
  ...
  useEffect(() => {
    ...
    await provider.send("eth_requestAccounts", []);   //metamask, kullanıcı hesabını bağlamak için izin istemeyi gerektirir
    }
  ...
  const handleGreetingSubmit = async (e) => {
     ...
     //ÜSTTEKİNİN YERİNE DÜZGÜN GÜNCELLENEBİLMESİ İÇİN :
